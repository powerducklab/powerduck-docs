...
这种划分对安全和可靠性很重要：需要网络或文件系统访问的请求由主进程处理，而不是网页视图，因此提示词和 API 密钥不会出现在浏览器控制台中，也不存在浏览器到服务商的直接 CORS 限制。参见 [AI 与模型](/docs/client/ai-models)。
...
你可以随意长期试用桌面客户端。它以一次性、**永久**授权的方式出售——一次购买，永久拥有，无需订阅。授权码在购买时生成并仅展示一次。Cloud 托管是另一项基于订阅的独立服务。两者的区别参见 [Cloud 账单](/docs/cloud/billing)。

下一步：进行[快速开始](/docs/overview/quickstart)。
